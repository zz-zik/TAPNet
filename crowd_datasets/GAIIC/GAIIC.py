# This file is used to crop the images and labels of GAIIC2024 dataset.
import os
import random
import re
...
class GAIIC(Dataset):
    def __init__(self, data_root, modal, rgb_transform=None, tir_transform=None, train=False, resizing=False,
        ...
        # TODO: 测试集路径
        self.rgb_labels_dir = os.path.join(self.data_dir, 'labels' if self.train else 'rgb_labels')  # 可见光标注文件路径
        self.tir_labels_dir = os.path.join(self.data_dir, 'labels' if self.train else 'tir_labels')  # 红外光标注文件路径
        self.offset = offset  # 是否对训练集进行偏移
        ...
        for filename in img_paths:
            ...
            rgb_labels_path = os.path.join(self.rgb_labels_dir,
                                           f"{filename.split('.')[0]}R.xml" if self.train else f"{filename.split('.')[0]}.xml")
            tir_labels_path = os.path.join(self.tir_labels_dir, f"{filename.split('.')[0]}R.xml")
            # 确保可见光与红外光图像都存在。
            if os.path.isfile(rgb_img_path) and os.path.isfile(tir_img_path) and os.path.isfile(
                    rgb_labels_path) and os.path.isfile(tir_labels_path):
                self.img_map[rgb_img_path] = (tir_img_path, rgb_labels_path, tir_labels_path)  # 存储可见光图像和红外光图像的对应关系
    ...
    def __getsingleitem__(self, index):
        ...
        # 获取 RGB 和 TIR 图像的路径及其注释
        rgb_img_path = self.img_list[index]
        tir_img_path, rgb_labels_path, tir_labels_path = self.img_map[rgb_img_path]

        # 加载 RGB 和 TIR 图像及其各自的注释
        rgb_img, rgb_points = load_data(rgb_img_path, rgb_labels_path)
        tir_img, tir_points = load_data(tir_img_path, tir_labels_path)

        # 如果定义了变换，则将其应用于 RGB 和 TIR 图像
...
def show_image_patches(rgb_patches, tir_patches, rgb_targets, tir_targets, num_patch=4):
    fig, axes = plt.subplots(num_patch, 2, figsize=(10, num_patch * 5))  # Create subplots for each patch (RGB and TIR)

    for i in range(num_patch):
        # Convert Tensor to NumPy and rearrange to (H, W, C)
        rgb_patch = rgb_patches[i].cpu().numpy().astype(np.uint8).transpose(1, 2, 0)  # Change shape to (H, W, C)
        tir_patch = tir_patches[i].cpu().numpy().astype(np.uint8).transpose(1, 2, 0)  # Change shape to (H, W, C)

        # Display RGB patch
        axes[i, 0].imshow(rgb_patch)
        for point in rgb_targets[i]['point']:
            rect = plt.Rectangle((point[0] - 1, point[1] - 1), 2, 2, fill=False, edgecolor='c', linewidth=2)
            axes[i, 0].add_patch(rect)
        axes[i, 0].set_title(f"RGB Patch {i + 1}")
        axes[i, 0].axis('off')

        # Display TIR patch
        axes[i, 1].imshow(tir_patch)
        for point in tir_targets[i]['point']:
            rect = plt.Rectangle((point[0] - 1, point[1] - 1), 2, 2, fill=False, edgecolor='c', linewidth=2)
            axes[i, 1].add_patch(rect)
        axes[i, 1].set_title(f"TIR Patch {i + 1}")
        axes[i, 1].axis('off')
...
if __name__ == '__main__':
    ...
    data_path = '/sxs/zhoufei/P2PNet/GAIIC2024'  # 输入文件路径
    ...
    train_dataset = GAIIC(data_path, modal='RT', rgb_transform=rgb_transform,
                          tir_transform=tir_transform, train=True, resizing=True, patch=True, flip=True,
                          offset=False)
    val_dataset = GAIIC(data_path, modal='RT', rgb_transform=rgb_transform,
    ...
    img_rgb, img_tir, target_rgb, target_tir = train_dataset[938]
    # show_image(img_rgb, img_tir, target_rgb[0]['point'], target_tir[0]['point'])
    show_image_patches(img_rgb, img_tir, target_rgb, target_tir, num_patch=4)
